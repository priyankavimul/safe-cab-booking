...
const Login = () => {
  ...
  const handleSocialLogin = (provider) => {
    // Faking a social bypass for instant login demonstrations
    const mockUser = {
      id: `social_${provider.toLowerCase()}_123`,
      name: `${provider} Demo User`,
      email: `demo@${provider.toLowerCase()}.com`
    };

    // Stash the mocked credentials
    localStorage.setItem('token', `mock_token_${provider.toLowerCase()}`);
